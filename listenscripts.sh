...
TEMP_ALARM_IDS=$(pgrep -c tempalarm)
TEMP_ALARM_PROC=$(pidof -x tempalarm.sh)
if [ "$TEMP_ALARM_IDS" -gt $MIN_ID ]; then
declare -a TEMP_ALARM_ARR
IFS=' ' read -r -a TEMP_ALARM_ARR <<< "$TEMP_ALARM__PROC"   
i=0
  while [ "${TEMP_ALARM_ARR[$i]}" != "${TEMP_ALARM_ARR[-1]}" ]; do
  echo "${TEMP_ALARM_ARR[$i]}"
          kill -9 "${TEMP_ALARM_ARR[$i]}"
          notify-send "tempalarm ${TEMP_ALARM_ARR[$i]} process ID is killed."
          i=$[$i +1]
  done
fi

if [ "$TEMP_ALARM_IDS" -eq $NO_ID ]; then
  notify-send "tempalarm is not running. Please check if tempalarm process is running" 
  if  tempalarm.sh & then
          notify-send "teampalarm is running"
  fi      
else 
  :
fi
