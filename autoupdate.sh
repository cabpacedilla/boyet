	...
	if [ "$UPGRADES" -gt 0 ]; then
	  	sudo apt update
			...
			if sudo apt -y upgrade; then
				...
				sudo apt autoclean
				notify-send --app-name "Auto-updates: $PACKAGES were updated."
			else
				notify-send --app-name "Auto-updates: Upgrade was unsuccessful."
			fi
	  	fi
	else
		notify-send --app-name "Auto-updates: System is up to date."
	fi
else
